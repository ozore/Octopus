...
Five artefacts. All are drafts. Slots in `{braces}` are rendered from `claims.json` or the corpus status record at publish time and are never typed by hand (`CORRECTIONS.md` §5, CL-1, CL-2).
...
>
> **About this note.** It was published by the same job that publishes the rate pages. There is no support queue behind it and replies to it are not delivered anywhere. One address does reach a mailbox — it is on the billing page, for payment disputes — and every message it receives is counted and published on `/status`.
...
### LP-2 — Directory listing fields (G2 Digital Markets family), if and only if the listing is free
...
| **Pricing** | Free tier, no account. $49 one-time bid rate card. $99 / $249 / $599 per month. No setup charge, no seats, no minimum term, no quote. |
...
Sitemap `lastmod` carries `last changed`, never `last verified` (`research/02-demand-seo.md` §8.2): a feed that claims thousands of changes on a night when nothing changed is a false machine-readable claim, and CL-2 is the check that catches it.
...
One page, linked from the footer of every marketing surface, that says plainly what kind of company this is. It is the permanent home of the standing disclosure specified in `community-playbook.md` §3, and it is what any of the four artefacts above points at when a reader wants to know who is talking.

---

## 5. Rules binding every piece of launch copy

1. **Mechanism, never outcome.** No accuracy, acceptance, coverage, time or human-minutes figure appears until its gate writes the flag (`CORRECTIONS.md` §4). Gate-locked sentences render from `claims.json`; typing one into a launch draft fails CL-1.
...
6. **Every surface states its own authorship** — that it was published automatically and that replies are not delivered — in the body, not behind a link (16 CFR 465.1(c)(4)).
...
- `run-2/phase-3-acquisition/outreach/community-playbook.md` — the standing disclosure LP-5 hosts
