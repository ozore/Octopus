...
| **Software review sites** | Capterra is reachable. Points North's **Certified Payroll Reporting** shows *"2 Reviews"*, 3.0/5. LCPtracker does not appear in Capterra's construction-management listing we sampled. G2, TrustRadius, SoftwareAdvice and GetApp all returned **403** here | Gartner Digital Markets (Capterra / SoftwareAdvice / GetApp) accepts a **free vendor-requested listing**; reviews must come from real users, which we do not yet have | **This is the only venue in the table with a legitimate no-human contribution:** a free, factual product listing. But a category whose incumbent carries two reviews is a *placement*, not a channel — it will not generate demand, it will only catch someone already comparing |
...
**The only community-shaped assets compatible with A1–A6**, all of which are things others can cite without us ever posting: the modification-diff pages and free WH-347 generator (T1); a machine-maintained public changelog / feed of newly published wage-determination revisions, which is a citable object rather than a message; and the per-artifact verification URL specified in `research/01-channels.md` §6. A citation by a forum member is community reach we did not have to be awake for. We do not plan against it, we instrument it — no coefficient exists yet.
...
**Pre-registered, per Ries.** Two thresholds, written now: (i) if the Gartner Digital Markets listing produces zero attributable sessions in its first 12 weeks live, it stays as a factual placement and gets no further engineering; (ii) if the prime→sub pages of §3.2 do not reach the same indexation bar T2 sets for wage-determination pages by week 16, they are deleted rather than defended — a page set justified by a feed we have just documented as incomplete has no second argument.
...
- `https://www.capterra.com/faq/faqs-vendors/` (via search summary; page 404 to us) and `https://launchdirectories.com/directory/capterra` — free vendor-requested listing syndicated across Capterra / Software Advice / GetApp
