...
1. **Time guarantee (unconditional, entirely in our control).** *"Your draft is in your inbox in 10 minutes or it's free."* It is cheap, it is differentiating, and **nobody else in the category offers a time guarantee** — so it is designed to lead. ⛔ **But it is gated, and it is currently OFF.** `ARCHITECTURE.md` §9 **G6** blocks this line from every surface until the automatic SLO-refund job runs in production and has been exercised on a deliberately-breached case. A guarantee is worth exactly the refund behind it; shipping the words first is the trust failure this whole brand is built to avoid. Until G6 clears, lead with the **revisions** guarantee and describe timing without a promise (*"written while you wait"*).
