...
5. **The 10-minute SLO is visible as a running clock, not a hidden SLA.** Because `paid_at → document_ready_at` is measured in code (`ARCHITECTURE.md §3.5`), the UI shows a real elapsed/target clock rather than a spinner — a status indicator that costs nothing to be honest about. **The refund *promise* attached to that clock is gated on G6** (`ARCHITECTURE.md` §9): until the automatic refund job is running, the clock reports time and claims nothing. Once G6 clears, the same component carries Hormozi's unconditional guarantee (§6.3) and does double duty.
