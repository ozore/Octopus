...
**Definition** (spec M11 §2): an org where at least one `comparisons` row exists against a certificate
**the org uploaded**, with the extraction out of `needs_review`. Emitted once per org, by the comparison
job, never by the UI.
...
infrastructure, not like a content subscription. 85% monthly logo retention is ~7.5 months of expected
life — at $99 that is roughly **$740 of gross revenue per customer**, which only supports acquisition
if CAC stays low, which is exactly what the outbound-only plan assumes.
...
Measured on the 16 real fixtures in `kb-samples/certificates/` (spec 03 §15). Reported as a per-field
table; **never averaged into one number**.

| measure | ship | block |
|---|---|---|
| **Critical fields exact** — `policy_exp`, `each_occurrence`, `general_aggregate`, `insured.name`, `addl_insd`, `subr_wvd` | **≥ 97%** | < 95% |
| **All fields** | **≥ 92%** | < 88% |
...
**Why 97% on critical fields and not 99%.** 16 fixtures × 6 critical fields ≈ 96 critical values; 97%
is roughly "at most three wrong". Demanding 99% on a set this size is demanding zero errors, which is
not a threshold — it is overfitting to sixteen documents. Demanding less than 95% ships a product whose
central claim is unreliable. The set grows with every customer document that exposes a new layout
(KB §E), and the gate rises with it.

**`policy_exp` is the field the entire product turns on.** A wrong expiry means a reminder ladder
scheduled against a date that does not exist, and a vendor marked covered while lapsed. It is the field
...
anonymous traffic. `OFFER.md` §4 budgets ~$0.50 per report; §5's own per-document figure implies
$2.50–5.00 for a full 25-document report. The two disagree, and M15 §11 resolves it with rate limits,
the Batch API and a **daily spend cap that disables new sessions** rather than overspending silently.
`gap_report_ready.cost_cents` retires the disagreement in week one, and the founder then decides
whether 25 certificates is the right free cap.
...
| **H-OF-1** | A card-required 14-day trial converts better here than a no-card one, as `OFFER.md` §9's sources say it does generally | untested for this buyer | §3, and `checkout_started → checkout_completed` (card abandonment) as the counter-signal |
