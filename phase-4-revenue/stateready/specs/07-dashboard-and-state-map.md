...
2. **The map.** US map, launch-15 states outlined. Each state the organisation operates in is filled
   by worst status inside it (green ok / amber ≤ 60 days / red ≤ 7 days or lapsed / grey not
   covered). Clicking a state filters everything below.
...
| screen | contents |
|---|---|
| `/dashboard?state=TX` | Same bands scoped to one state; the map keeps the selection. |
...
export const dashboardSummaries = pgTable("dashboard_summaries", {
  ...
  byState:        jsonb("by_state").notNull(),   // { TX: {ok, amber, red, lapsed, covered}, … }
  counts:         jsonb("counts").notNull(),     // { licences, technicians, deadlines30, deadlines7, lapsed }
  worstStatus:    text("worst_status", { enum: ["ok","amber","red","lapsed"] }).notNull(),
});
```
...
- Status thresholds are constants, not magic numbers scattered in components: `RED ≤ 7`,
  `AMBER ≤ 60`, else green; lapsed is its own state.
...
2. An organisation with one lapsed Texas plumbing licence shows the red status line naming the state
   and the holder, above the fold, on a 1366×768 screen.
3. Clicking Texas filters the deadline cards and the URL is shareable.
...
- **A state the organisation does not operate in but holds a licence in.** Shown on the map anyway,
...
- **Everything green.** The status line must still say something useful: "Nothing due in the next 60
...
- **300 licences in 20 states.** The map is fine; the cards must paginate and default to 30 days.
- **Two entities with the same licence type in the same state.** Cards show the entity name; the map
  aggregates.
- **Colour-blind users.** Status is never colour alone: every chip carries a shape and a word.

## Errors

| condition | user sees |
|---|---|
| Summary computation fails | Fall back to a live query with a "showing live figures" note, and flag internally. Never a blank map. |
...
- **Accessibility:** axe pass on the dashboard; map has a table equivalent; no colour-only status.
