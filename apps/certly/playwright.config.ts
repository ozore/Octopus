...
const PORT = Number(process.env['E2E_PORT'] ?? 3125);
/**
...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      PORT: String(PORT),
      // Turns off the dev overlay portal, which sits on top of the nav's
      // Sign out button (see next.config.mjs).
      E2E: '1',
      ...
      APP_NAME: 'Certly',
      APP_SLUG: 'certly',
      // Distinct per app (DEPLOY_VERCEL.md §3), and exercised here so the
      // middleware and getSession() are proved to agree on the name.
      ...
      STRIPE_PRICE_CERTLY_STARTER_MONTHLY: 'price_e2e_starter',
      STRIPE_PRICE_CERTLY_STANDARD_MONTHLY: 'price_e2e_standard',
      STRIPE_PRICE_CERTLY_PORTFOLIO_MONTHLY: 'price_e2e_portfolio',
    },
  },
});
