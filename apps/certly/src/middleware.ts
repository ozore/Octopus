...
const PROTECTED = ['/dashboard', '/settings', '/vendors', '/requirements', '/review'];

export function middleware(request: NextRequest) {
  const { pathname } = request.nextUrl;
  // THE DEFAULT MUST MATCH THE PLATFORM'S (`packages/platform/src/env.ts`).
  // The middleware reads the cookie by name and `getSession()` reads it by the
  // same name; if the two defaults differ and the variable is unset, the
  // middleware sees no cookie, redirects to /login, /login sees a live session
  // and redirects to /dashboard — ERR_TOO_MANY_REDIRECTS, with nothing in any
  // log to explain it. `.env.example` sets `certly_session`, which is what
  // DEPLOY_VERCEL.md asks for when apps share a domain; the DEFAULT is the
  // platform's, so an unset variable can never produce the loop.
