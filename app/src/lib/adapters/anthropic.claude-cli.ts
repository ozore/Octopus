...
export class ClaudeCliAnthropicAdapter implements AnthropicAdapter {
  ...
  async runCited(req: CitedRequest): Promise<CitedResponse> {
    const prompt = [
      ...
      renderDocuments(req.documents),
      req.userText,
      '\n\nDo not use any tools. Respond with ONLY a single JSON value (no prose, no',
      'markdown fence) of this exact shape:',
      '{"blocks":[{"text":"<a passage of the document you are writing>",',
      '"citations":[{"documentId":"<id from the DOCUMENTS list>","blockIndex":<number>,',
      '"quote":"<EXACT verbatim substring copied from that block>"}]}]}',
      'Every factual claim about policy MUST carry at least one citation whose',
      '"quote" is copied character-for-character from the referenced block —',
      'quotes are machine-verified against the source and any mismatch voids the',
      `citation. Only these document ids are citable: ${req.citableDocumentIds.join(', ')}.`,
    ].join('\n');
