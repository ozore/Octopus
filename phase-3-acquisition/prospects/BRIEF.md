...
7. **Known blocked from this environment (do not retry):** reddit.com, yelp.com, importyeti.com, facebook.com, duckduckgo.com, clutch.co, designrush.com, realself.com, zocdoc.com, caionline.org, IREM chapter directories. Google search redirects. Bing via curl returns degraded or unrelated results through the proxy: use the WebSearch tool instead. narpm.org answers to `curl -sL --compressed` with a Chrome user agent, but its chapter list renders client-side (fleet finding, 2026-09-03).
8. **What works:** the WebSearch tool; sitemaps (`/page-sitemap.xml`, `/sitemap_index.xml`) when a directory renders client-side; JSON embedded in search HTML (bbb.org) or in Next.js payloads (`self.__next_f.push`); a2z exhibitor directories (`*.a2zinc.net`); public government APIs (CPSC recall API at saferproducts.gov, api.usaspending.gov); WebFetch for reading articles; `curl -s -A "Mozilla/5.0" <url>` for pulling directory pages you then parse with python3 (WebFetch summarises through a small model and drops rows, so for list pages always curl + parse); public government APIs (api.usaspending.gov POST endpoints, sam.gov search API, state licence lookups); Bing search results via curl.
