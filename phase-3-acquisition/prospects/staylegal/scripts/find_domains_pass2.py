...
if not os.path.isdir(BASE):
    BASE = "."
DEFAULT_ARGS = [os.path.join(BASE, 'raw/names_miss.json'), os.path.join(BASE, 'raw/domains3.json')]
...
def candidates(name):
    ...
    outs.append("-".join(t))
    # widen the net for names whose domain adds or drops a trailing word
    for suf in ("vacations", "rentals", "stays", "homes", "properties", "co"):
        outs.append(j + suf)
    if len(t) > 1:
        outs.append("".join(t[:-1]))
        outs.append("".join(t[1:]))
    seen, urls = set(), []
    for base in outs:
